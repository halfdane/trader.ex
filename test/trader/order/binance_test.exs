defmodule Trader.Order.BinanceTest do
  ...
  test "create query string" do
    assert Binance.create_query_string(@params) == "symbol=LTCBTC&side=BUY&type=LIMIT&timeInForce=GTC&quantity=1&price=0.1&recvWindow=5000&timestamp=1499827319559"
  end

  test "calculate signature" do
    query = Binance.create_query_string(@params)

    assert Binance.calculate_signature(query, @binance_access) == "c8db56825ae71d6d79447849e617115f4a920fa2acdcab2b053c4b2838bd6b71"
  end

  test "complete query" do
    signed = Binance.sign(@params, @binance_access)

    assert signed.query == "symbol=LTCBTC&side=BUY&type=LIMIT&timeInForce=GTC&quantity=1&price=0.1&recvWindow=5000&timestamp=1499827319559&signature=c8db56825ae71d6d79447849e617115f4a920fa2acdcab2b053c4b2838bd6b71"
  ...
  test "sign only if access info given" do
    ...
    assert signed.query == "symbol=LTCBTC&side=BUY&type=LIMIT&timeInForce=GTC&quantity=1&price=0.1&recvWindow=5000&timestamp=1499827319559"
